...
while ct<=3*H/deltam
    ...
    if ct > H/deltam
        dt = tfrac*(H/deltam);
        phi(2) = phi0*(1 + purt*cos(2*pi*(ct)/(lambda1*cnumber))...
    ...
    %Boundary conditions for gas exsolution
        Param.rhophi.dof_dir        = [GridS.dof_xmin];
        Param.rhophi.dof_f_dir      = [GridS.dof_f_xmin];     % identify faces on Dirichlet bnd
        Param.rhophi.g              = rhophibc;%rhobc*phi(1);                   % set Dirichlet Bnd
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Param.rhophi.dof_neu        = [];  % identify cells on Neumann bnd
        Param.rhophi.dof_f_neu      = [];  % identify cells on Neumann bnd
        Param.rhophi.qb             = [];
    ...
    phi(1)=2*phi(2)-phi(3);
    rhog(2) = rho0;
    DeltaW(2) = DeltaW(3); DeltaW(1) = DeltaW(2);
...
save('testfile.mat','GridS','dt','phi','phiM',...
